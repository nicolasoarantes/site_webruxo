...
console.log("Jogo Wander Webruxo - Script Carregado v1.2 - Mira e Gancho Aprimorados");
...
const COLORS = {
    ...
    AIM_INDICATOR_HIT: 0x39FF14, // Verde Neon para indicar alvo válido
...
let potentialHookTarget = null; // Para indicar alvo antes de atirar
...
let aimPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0);

// --- Inicialização --- 
function init() {
    console.log("Inicializando o jogo com Mira e Gancho Aprimorados...");
    ...
    window.addEventListener('game-pause-toggle', (event) => {
        ...
        if (gamePaused) {
            clock.stop();
            if (document.pointerLockElement) document.exitPointerLock(); // Liberar mouse ao pausar
        } else {
            clock.start();
            if (gameStarted && !isMobile) renderer.domElement.requestPointerLock(); // Travar mouse ao resumir
        }
    });

    loadingScreen.style.display = 'none';
    console.log("Jogo inicializado com Mira e Gancho Aprimorados.");
}
...
function startGame() {
    ...
    gameStarted = true;
    gamePaused = false; // Garantir que não comece pausado
    if (!isMobile) {
...
function createAimIndicator() {
    // Geometria: um ponto e uma linha
    const indicatorGroup = new THREE.Group();

    // Linha da mira
    const lineGeometry = new THREE.BufferGeometry();
    const lineVertices = new Float32Array([0, 0, 0, 0, 0, -hookMaxDistance]); // Linha aponta para frente
    ...
    // Ponto/Círculo no final da mira
    ...
    point.position.z = -hookMaxDistance; // Posiciona no final da linha
    point.rotation.x = Math.PI / 2; // Orienta o torus corretamente
...
function setupControls() {
    ...
    if (!isMobile) {
        ...
        renderer.domElement.addEventListener('mousedown', (event) => { // Usar mousedown para ativar gancho
            if (gamePaused || event.button !== 0) return; // Apenas botão esquerdo
        ...
        renderer.domElement.addEventListener('mouseup', (event) => { // Usar mouseup para desativar
...
// --- Atualizar Direção da Mira e Indicador Visual ---
function updateAimDirection() {
    if (!player || !aimIndicator) return;

    // Calcular direção baseada no mouse
    ...
    // Atualizar posição e orientação do indicador de mira
    const aimOrigin = player.position.clone().add(new THREE.Vector3(0, 0.5, 0));
    aimIndicator.position.copy(aimOrigin);
    aimIndicator.lookAt(aimOrigin.clone().add(aimDirection));
    ...
    // Verificar se há um alvo potencial para o gancho na direção da mira
    potentialHookTarget = null;
    if (aimIndicator.visible && !hookActive) {
        raycaster.set(aimOrigin, aimDirection);
    ...
    // Atualizar cor do indicador de mira
...
// --- Lógica do Gancho Mágico (Usando Alvo Potencial) ---
function activateHook() {
    // Usar o alvo potencial já calculado pela mira
    if (potentialHookTarget) {
        ...
        if (aimIndicator) aimIndicator.visible = false; // Esconder mira ao ativar gancho
    } else {
        console.log("Nenhum ponto de ancoragem válido na direção da mira.");
    }
}
...
function deactivateHook() {
    ...
    if (aimIndicator && !isMobile && document.pointerLockElement) aimIndicator.visible = true; // Mostrar mira novamente se mouse travado
...
function updatePlayerMovement(delta) {
    ...
    if (!hookActive) {
        ...
        if (isTouching) { moveInput.add(cameraDirection.clone().multiplyScalar(-touchMoveY * 50)); moveInput.add(rightDirection.clone().multiplyScalar(-touchMoveX * 50)); horizontalInput = Math.abs(touchMoveX) > 0.001 || Math.abs(touchMoveY) > 0.001; }
...
function updateCamera(delta) {
    ...
    const playerRotation = new THREE.Quaternion(); player.getWorldQuaternion(playerRotation);
    ...
    if (cameraIntersects.length > 0) cameraDistance = Math.min(cameraDistance, cameraIntersects[0].distance * 0.9);
