...
console.log("Jogo Wander Webruxo - Script Carregado v1.4 - Câmera e Gancho Refinados");
...
const hookPullAcceleration = 60; // Aumentada para mais "puxão"
const hookMaxPullSpeed = 30; // Aumentada
const hookMinDistance = 1.5; // Aumentada ligeiramente para evitar ficar "colado"
let raycaster = new THREE.Raycaster();
let potentialHookTarget = null;
let hookCooldown = 0; // Cooldown para evitar ativação/desativação muito rápida
const hookCooldownTime = 0.2; // 200ms
...
// --- Inicialização --- 
function init() {
    console.log("Inicializando o jogo com Câmera e Gancho Refinados...");
    ...
    console.log("Jogo inicializado com Câmera e Gancho Refinados.");
}
...
function createAimIndicator() {
    ...
    // Linha mais curta para ser menos intrusiva
    ...
    point.position.z = -hookMaxDistance * 0.8; // Posiciona no final da linha curta
...
function updateAimIndicatorVisuals() {
    ...
    aimIndicator.visible = !isMobile && document.pointerLockElement === renderer.domElement && !hookActive; // Esconder se gancho ativo

    potentialHookTarget = null;
    if (aimIndicator.visible) { // Verifica apenas se o indicador está visível
    ...
    if (aimLine && aimPoint) {
        ...
        pointMaterial.color.setHex(targetColor);
    }
}
...
function activateHook() {
    if (potentialHookTarget && hookCooldown <= 0) {
        ...
        playerVelocity.set(0, 0, 0); // Parar movimento ao ancorar
        onGround = false;
        createHookActivationEffect();
        if (aimIndicator) aimIndicator.visible = false; // Esconder mira
    } else {
...
function deactivateHook() {
    if (hookCooldown <= 0) {
        ...
        if (aimIndicator && !isMobile && document.pointerLockElement) aimIndicator.visible = true; // Mostrar mira novamente
        // Impulso leve ao soltar
        const cameraDirection = camera.getWorldDirection(new THREE.Vector3());
        playerVelocity.add(cameraDirection.multiplyScalar(jumpForce * 0.2)); 
        playerVelocity.y = Math.max(playerVelocity.y, jumpForce * 0.3); // Garante um impulso vertical mínimo
    }
}
...
function updateHook(delta) {
    ...
    if (hookActive && hookTarget && hookLine) {
        ...
        positions.setXYZ(1, hookTarget.x, hookTarget.y, hookTarget.z); // Garantir que o ponto final esteja correto
        positions.needsUpdate = true;
        ...
        if (distanceToTarget > hookMinDistance) {
            ...
            // Adicionar um pouco de gravidade simulada para arco
            playerVelocity.y += gravity * delta * 0.3; 
            if (Math.random() > 0.7) createHookParticle();
        } else {
            // Chegou ao destino ou muito perto
            deactivateHook();
            playerVelocity.multiplyScalar(0.5); // Reduzir velocidade ao chegar
        }
    } else {
...
function updatePlayerMovement(delta) {
    ...
    // Aplicar gravidade apenas se não estiver usando o gancho
    if (!hookActive) {
    ...
    platformColliders.forEach((platformBox, index) => {
        if (playerCollider.intersectsBox(platformBox)) {
            ...
            if (overlapY < overlapX && overlapY < overlapZ) {
                if (player.position.y > (platformMin.y + platformMax.y) / 2 && playerVelocity.y <= 0) { // Apenas colidir por cima se caindo
                    player.position.y = platformMax.y + (playerMax.y - player.position.y) + 0.01;
                    if (playerVelocity.y < 0) { 
                        onGround = true; 
                        lastPlatform = platform; 
                        // Aplicar velocidade da plataforma apenas se não estiver usando gancho
                        if (!hookActive) {
                    ...
                    playerVelocity.y = 0;
                } else if (player.position.y <= (platformMin.y + platformMax.y) / 2 && playerVelocity.y > 0) { // Apenas colidir por baixo se subindo
...
function updateCamera(delta) {
    ...
    const finalCameraOffset = cameraRayDirection.multiplyScalar(actualCameraDistance); // Usar direção normalizada
    const finalCameraPosition = cameraRayOrigin.clone().add(finalCameraOffset); // Calcular a partir da origem do raio
