...
console.log("Jogo Wander Webruxo - Script Carregado v1.5 - Correções de Bugs e Retorno");
...
const backToSiteButton = document.getElementById('back-to-site-button'); // Novo botão
...
// --- Inicialização --- 
function init() {
    console.log("Inicializando o jogo v1.5...");
    ...
    createAimIndicator();
    setupControls();
    setupMenuButtons(); // Configurar botões do menu

    loadingScreen.style.display = 'none';
    instructionScreen.style.display = 'flex'; // Mostrar instruções primeiro
    console.log("Jogo inicializado v1.5. Aguardando início.");
}

// --- Função para iniciar o jogo (Corrigida) ---
function startGame() {
    console.log("Iniciando o jogo...");
    if (gameStarted) return; // Evitar múltiplos inícios

    instructionScreen.style.display = 'none';
    configScreen.style.display = 'none';
    pauseMenu.style.display = 'none';
    pauseButton.style.display = 'flex'; // Mostrar botão de pausa

    gameStarted = true;
    gamePaused = false;
    clock.start(); // Iniciar ou reiniciar o relógio

    // Resetar estado do jogador se necessário
    ...
    // Solicitar Pointer Lock APÓS um pequeno delay para garantir que a tela de instruções sumiu
    setTimeout(() => {
    ...
    // Iniciar o loop de animação se não estiver rodando
...
function togglePause(pauseState) {
    ...
    pauseButton.innerHTML = gamePaused ? '▶' : 'II'; // Mudar ícone do botão
    ...
    if (gamePaused) {
        ...
        clock.start();
        if (!isMobile) {
            // Tentar obter pointer lock novamente ao despausar
            renderer.domElement.requestPointerLock();
        }
        // Reiniciar o loop de animação se parou
        if (animationFrameId === null) {
            animate();
        }
    }
...
function setupMenuButtons() {
    ...
    restartButton.addEventListener('click', () => {
        togglePause(false); // Garante que o jogo está despausado
        startGame(); // Reinicia o jogo
    });
    exitButton.addEventListener('click', () => {
        // Simplesmente volta para a tela de instruções
        gameStarted = false;
        gamePaused = true;
    ...
    backToSiteButton.addEventListener('click', () => {
        // Tenta navegar a janela pai (se estiver em iframe)
        if (window.parent && window.parent.location !== window.location) {
            window.parent.location.href = '../index.html'; // Ajuste o caminho se necessário
        } else {
            // Fallback se não estiver em iframe (ou erro)
    ...
    saveConfigButton.addEventListener('click', () => {
        // Lógica para salvar config (se implementada)
        configScreen.style.display = 'none';
        if (!gameStarted) {
            instructionScreen.style.display = 'flex';
        } else {
            togglePause(false); // Volta ao jogo se estava pausado
        }
    });
    backButton.addEventListener('click', () => {
        ...
        if (!gameStarted) {
            ...
            pauseMenu.style.display = 'flex'; // Volta ao menu de pausa
        }
    });
}
...
function createAimIndicator() {
    ...
    point.position.z = -hookMaxDistance * 0.8;
...
function setupControls() {
    console.log("Configurando controles v1.5...");
    const keyConfig = window.gameKeyConfig || {}; // Usar config global se existir

    document.addEventListener('keydown', (event) => {
        const key = event.key.toLowerCase();
        // Ignorar input se estiver configurando teclas
        ...
        if (key === (keyConfig.pause?.key || 'escape') || key === 'p') {
            if (gameStarted) {
                ...
                return; // Não processar outras teclas se pausou/despausou
            }
        }
        
        if (gamePaused) return; // Ignorar outras teclas se pausado
    ...
    if (!isMobile) {
        ...
        renderer.domElement.addEventListener('mousedown', (event) => {
            ...
            if (!document.pointerLockElement) {
                // Tentar obter lock apenas se o jogo já começou
                if (gameStarted) {
                    renderer.domElement.requestPointerLock();
                }
            } else {
        ...
        document.addEventListener('pointerlockchange', () => {
            if (document.pointerLockElement === renderer.domElement) {
                ...
                // Se o jogo estava pausado por perda de foco, despausar
                ...
                // Pausar o jogo se o lock for perdido e o jogo estiver rodando
                if (gameStarted && !gamePaused) {
                    togglePause(true);
                }
            }
...
// --- Handler de Movimento do Mouse (controla a câmera) ---
function onMouseMove(event) {
    // Só processar se o pointer lock estiver ativo e o jogo não estiver pausado
...
function updateAimIndicatorVisuals() {
    ...
    // Mostrar indicador apenas se o pointer lock estiver ativo, não for mobile e o gancho não estiver ativo
    ...
    if (aimLine && aimPoint) {
        ...
        // Certificar que os materiais existem antes de setar a cor
        if (aimLine.material) aimLine.material.color.setHex(targetColor);
        if (aimPoint.material) aimPoint.material.color.setHex(targetColor);
    }
}
...
function createJumpEffect() {
    ...
    function animateJumpParticles() {
        if (!gameStarted) return; // Parar se o jogo foi resetado
...
function animate() {
    // Parar o loop se o jogo não começou ou foi explicitamente parado
    if (!gameStarted) {
        animationFrameId = null;
        return;
    }
    ...
    // Não processar lógica se pausado
    ...
    platformColliders.forEach((platformBox, index) => {
        if (playerCollider.intersectsBox(platformBox)) {
            ...
            if (overlapY < overlapX && overlapY < overlapZ) {
                if (player.position.y > (platformMin.y + platformMax.y) / 2 && playerVelocity.y <= 0) {
                    ...
                    playerVelocity.y = 0;
                } else if (player.position.y <= (platformMin.y + platformMax.y) / 2 && playerVelocity.y > 0) {
