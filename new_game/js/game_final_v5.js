...
console.log("Jogo Wander Webruxo - Script Carregado v1.6 - Câmera Invertida e Mira Aprimorada");
...
const COLORS = {
    ...
    GLOW: 0xFF00FF,
    AIM_INDICATOR: 0xFF00FF, // Magenta padrão
    AIM_INDICATOR_HIT: 0x39FF14, // Verde neon ao acertar
    AIM_INDICATOR_MISS: 0xFF6B35, // Laranja se não houver alvo
...
// --- Inicialização --- 
function init() {
    console.log("Inicializando o jogo v1.6...");
    ...
    console.log("Jogo inicializado v1.6. Aguardando início.");
}
...
function createAimIndicator() {
    ...
    // Linha mais fina e com gradiente de opacidade
    ...
    // Ponto alvo mais distinto (esfera pequena)
...
function setupControls() {
    console.log("Configurando controles v1.6...");
...
// --- Handler de Movimento do Mouse (Câmera Invertida) ---
function onMouseMove(event) {
    if (document.pointerLockElement === renderer.domElement && !gamePaused) {
        // Invertido: movimento do mouse para a direita -> câmera para a direita
        cameraYaw += event.movementX * mouseSensitivity; 
        // Invertido: movimento do mouse para cima -> câmera para cima
...
function updateAimIndicatorVisuals() {
    ...
    let targetColor = COLORS.AIM_INDICATOR_MISS; // Cor padrão: laranja (sem alvo)
    ...
    if (aimIndicator.visible) {
        ...
        if (potentialHookTarget) {
            targetColor = COLORS.AIM_INDICATOR_HIT; // Verde se houver alvo
            targetPointScale = 1.5; // Ponto maior ao mirar em alvo
            targetLineOpacity = 0.8; // Linha mais opaca
        }
    }

    // Atualizar cor e aparência da mira
...
// --- Atualização do Movimento do Jogador (Câmera Invertida) --- 
function updatePlayerMovement(delta) {
    ...
    // Ajuste para câmera invertida: rightDirection agora é calculado com base na câmera
    ...
    if (!hookActive) {
        ...
        // W/S agora movem na direção da câmera (frente/trás)
        ...
        // A/D agora movem perpendicular à direção da câmera (esquerda/direita)
...
// --- Atualização da Câmera (com controle de mouse invertido) ---
function updateCamera(delta) {
    ...
    // Cálculo do offset da câmera baseado em yaw e pitch
    ...
    // Raycasting para evitar que a câmera entre nas plataformas
