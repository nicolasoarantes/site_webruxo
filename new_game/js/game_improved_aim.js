...
console.log("Jogo Wander Webruxo - Script Carregado v1.1 - Mira com Mouse");
...
const COLORS = {
    // Cores Primárias
    PLAYER_BODY: 0xFFD700, // Amarelo Dourado
    PLAYER_HAT: 0x6A1B9A,  // Roxo Webruxo
    PLAYER_SHOES: 0x00FFFF, // Ciano Neon

    // Cores de Fundo e Ambiente
    BACKGROUND_TOP: 0x0B0B2D, // Azul Noturno
    BACKGROUND_BOTTOM: 0x2D0B38, // Roxo Profundo
    FOG: 0x2D0B38, // Roxo Profundo

    // Cores de Plataformas
    PLATFORM_BASE: 0x0B6EFD, // Azul Elétrico
    PLATFORM_ACCENT: 0x39FF14, // Verde Neon
    PLATFORM_SPECIAL: 0xFF6B35, // Laranja Vibrante

    // Efeitos e Elementos Especiais
    HOOK_LINE: 0xFF00FF, // Rosa Neon
    COLLECTIBLE: 0x00FFFF, // Ciano Neon
    GLOW: 0xFF00FF, // Rosa Neon para efeitos de brilho
    AIM_INDICATOR: 0xFF00FF, // Rosa Neon para mira
...
let gamePaused = false; // Variável de pausa
...
// Variáveis de Mira com Mouse
let mouse = new THREE.Vector2();
let aimDirection = new THREE.Vector3();
let aimIndicator = null;
let aimPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0); // Plano para calcular ponto de mira

// --- Inicialização --- 
function init() {
    console.log("Inicializando o jogo com Mira com Mouse...");
    ...
    createAimIndicator(); // Criar indicador de mira
    setupControls();

    startGameButton.addEventListener('click', startGame);

    // Escutar evento de pausa do HTML
    ...
    console.log("Jogo inicializado com Mira com Mouse.");
}
...
function startGame() {
    ...
    gameStarted = true;
    if (!isMobile) {
        // Travar ponteiro do mouse para controle de mira
...
function createPlayer() {
    ...
    const hatMaterial = new THREE.MeshStandardMaterial({ 
        color: COLORS.PLAYER_HAT, 
        roughness: 0.3, 
        metalness: 0.7,
        emissive: COLORS.PLAYER_HAT,
    ...
    const shoeMaterial = new THREE.MeshStandardMaterial({ 
        color: COLORS.PLAYER_SHOES, 
        roughness: 0.5, 
        metalness: 0.5,
        emissive: COLORS.PLAYER_SHOES,
        emissiveIntensity: 0.2
    });
...
function createPlatformGlow(geometry, position) {
    ...
    if (geometry instanceof THREE.BoxGeometry) {
        glowGeometry = new THREE.BoxGeometry(
            geometry.parameters.width * 1.05,
            geometry.parameters.height * 1.05,
            geometry.parameters.depth * 1.05
        );
    } else if (geometry instanceof THREE.CylinderGeometry) {
        glowGeometry = new THREE.CylinderGeometry(
            geometry.parameters.radiusTop * 1.05,
            geometry.parameters.radiusBottom * 1.05,
            geometry.parameters.height * 1.05,
            geometry.parameters.radialSegments
        );
    } else {
        glowGeometry = geometry.clone();
    }
...
function createBackgroundElements() {
    ...
    mountains.position.set(0, -5, -100);
    scene.add(mountains);
    ...
    const shapes = [
        ...
        new THREE.TorusGeometry(4, 0.5, 8, 16)
    ];
    for (let i = 0; i < 20; i++) {
        ...
        element.userData.rotationSpeed = {
...
function createParticleSystem() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        particlePositions[i3 + 1] = (Math.random() - 0.5) * 100 + 20;
        particlePositions[i3 + 2] = (Math.random() - 0.5) * 200 - 50;
        particleSizes[i] = Math.random() * 2 + 0.5;
        if (i % 3 === 0) {
            particleColors[i3] = 1; particleColors[i3 + 1] = 0; particleColors[i3 + 2] = 1;
        } else if (i % 3 === 1) {
            particleColors[i3] = 0; particleColors[i3 + 1] = 1; particleColors[i3 + 2] = 0.5;
        } else {
...
function createAimIndicator() {
    ...
    const indicatorMaterial = new THREE.LineBasicMaterial({ 
        color: COLORS.AIM_INDICATOR, 
        linewidth: 2, 
        transparent: true, 
        opacity: 0.7 
    });

    aimIndicator = new THREE.Line(indicatorGeometry, indicatorMaterial);
    aimIndicator.scale.set(1, 1, 2); // Comprimento inicial da linha
    aimIndicator.visible = false; // Começa invisível
...
function setupControls() {
    ...
    const keyConfig = window.gameKeyConfig || {}; // Obter config do HTML
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (key === (keyConfig.hook?.key || 'shift') || key === 'f') {
            if (!hookActive) activateHook();
        }
        if (key === (keyConfig.jump?.key || ' ') && onGround) {
    ...
    // Controle de mouse para mira (apenas desktop)
    if (!isMobile) {
        ...
        // Lidar com saída do pointer lock
        document.addEventListener('pointerlockchange', () => {
            if (!document.pointerLockElement && gameStarted && !gamePaused) {
                // Pausar o jogo se o ponteiro for liberado
    ...
    if (isMobile) {
        ...
        hookButton.addEventListener('touchstart', (e) => {
            e.preventDefault();
            if (!hookActive && !gamePaused) activateHook();
        }, { passive: false });
        hookButton.addEventListener('touchend', (e) => {
            e.preventDefault();
            if (hookActive && !gamePaused) deactivateHook();
        }, { passive: false });

        gameContainer.addEventListener('touchstart', (e) => {
            if (e.target === jumpButton || e.target === hookButton || gamePaused) return;
            isTouching = true;
...
function onMouseMove(event) {
    if (document.pointerLockElement === renderer.domElement) {
        // Calcular movimento relativo do mouse
...
function updateAimDirection() {
    ...
    // Atualizar plano de mira para estar na frente da câmera
    aimPlane.setFromNormalAndCoplanarPoint(camera.getWorldDirection(aimPlane.normal), player.position);

    // Lançar raio da câmera através da posição do mouse
    raycaster.setFromCamera(mouse, camera);

    // Encontrar ponto de interseção com o plano de mira
    const intersectPoint = new THREE.Vector3();
    raycaster.ray.intersectPlane(aimPlane, intersectPoint);

    if (intersectPoint) {
        // Calcular direção do jogador para o ponto de interseção
        aimDirection.subVectors(intersectPoint, player.position).normalize();
    } else {
        // Se não houver interseção (raro), usar direção da câmera
        camera.getWorldDirection(aimDirection);
    }

    // Atualizar indicador visual
    if (aimIndicator) {
        aimIndicator.position.copy(player.position).add(new THREE.Vector3(0, 0.5, 0)); // Posição inicial da mira
        aimIndicator.lookAt(aimIndicator.position.clone().add(aimDirection));
        aimIndicator.visible = true; // Tornar visível quando o mouse se move
    }
}
...
function createJumpEffect() {
    ...
    for (let i = 0; i < jumpParticleCount; i++) {
        ...
        jumpParticlePositions[i3 + 1] = player.position.y;
    ...
    function animateJumpParticles() {
        if (!gameStarted) return; // Parar se o jogo não começou
...
// --- Lógica do Gancho Mágico (Refinada com Mira) ---
function activateHook() {
    console.log("Tentando ativar gancho com mira...");

    // Usar aimDirection calculada pelo mouse (ou direção da câmera como fallback)
    const hookDirection = aimDirection.length() > 0.1 ? aimDirection.clone() : camera.getWorldDirection(new THREE.Vector3());

    const rayOrigin = player.position.clone().add(new THREE.Vector3(0, 1.0, 0));
    raycaster.set(rayOrigin, hookDirection);
    ...
    intersects.forEach(intersect => {
        if (intersect.object.userData.isAnchorable && 
            intersect.distance <= minDistance && 
            intersect.distance > hookMinDistance) {
    ...
    if (closestAnchor) {
        ...
        console.log("Nenhum ponto de ancoragem encontrado na direção da mira.");
    }
}
...
function createHookActivationEffect() {
    ...
    for (let i = 0; i < ringParticleCount; i++) {
        ...
        const radius = 0.5;
        const i3 = i * 3;
        ringPositions[i3] = hookTarget.x + Math.cos(angle) * radius;
        ringPositions[i3 + 1] = hookTarget.y;
        ringPositions[i3 + 2] = hookTarget.z + Math.sin(angle) * radius;
    ...
    function animateRingParticles() {
        ...
        for (let i = 0; i < ringParticleCount; i++) {
            ...
            positions[i3] = hookTarget.x + Math.cos(angle) * radius;
            positions[i3 + 1] = hookTarget.y;
            positions[i3 + 2] = hookTarget.z + Math.sin(angle) * radius;
        }
...
function createHookParticle() {
    ...
    positions[0] = particlePos.x; positions[1] = particlePos.y; positions[2] = particlePos.z;
    ...
    function animateParticle() {
        if (!gameStarted) return;
        const elapsed = clock.elapsedTime - startTime;
        if (elapsed > 0.3 || !hookActive) {
            scene.remove(particle);
            return;
        }
        particleMaterial.opacity = 1 - elapsed * 3;
        particleMaterial.size = 0.3 * (1 - elapsed);
...
function animate() {
    ...
    if (gamePaused) return; // Pular atualização se pausado
    ...
    updateAimDirection(); // Atualizar mira constantemente
...
function updatePlayerMovement(delta) {
    ...
    if (!hookActive) {
        ...
        if (keys[keyConfig.left?.key || 'a'] || keys['arrowleft']) { moveInput.add(rightDirection); horizontalInput = true; }
        if (keys[keyConfig.right?.key || 'd'] || keys['arrowright']) { moveInput.sub(rightDirection); horizontalInput = true; }
        ...
        if (horizontalInput) {
            moveInput.normalize();
            targetVelocity.add(moveInput.multiplyScalar(moveSpeed));
        }
    }
    ...
    if (!hookActive) {
        ...
        if (velocityDifference.length() > 0.001) {
            ...
            if (step.length() > velocityDifference.length()) {
                playerVelocity.x = targetVelocity.x;
                playerVelocity.z = targetVelocity.z;
            } else {
                playerVelocity.x += step.x;
                playerVelocity.z += step.z;
            }
        }
    }
...
function handleCollisions(delta) {
    ...
    platformColliders.forEach((platformBox, index) => {
        if (playerCollider.intersectsBox(platformBox)) {
            ...
            const playerMin = playerCollider.min;
            const playerMax = playerCollider.max;
            ...
            if (overlapY < overlapX && overlapY < overlapZ) {
                ...
                if (player.position.x < (platformMin.x + platformMax.x) / 2) {
                    player.position.x = platformMin.x - (playerMax.x - player.position.x) - 0.01;
                } else {
                    player.position.x = platformMax.x + (player.position.x - playerMin.x) + 0.01;
                }
                playerVelocity.x = 0;
            } else {
                if (player.position.z < (platformMin.z + platformMax.z) / 2) {
                    player.position.z = platformMin.z - (playerMax.z - player.position.z) - 0.01;
                } else {
                    player.position.z = platformMax.z + (player.position.z - playerMin.z) + 0.01;
                }
                playerVelocity.z = 0;
            }
            updatePlayerCollider();
...
function updateCamera(delta) {
    ...
    const cameraRay = new THREE.Raycaster(
        ...
        desiredCameraPosition.clone().sub(cameraTargetPosition).normalize()
    );
    ...
    if (cameraIntersects.length > 0) {
        cameraDistance = Math.min(cameraDistance, cameraIntersects[0].distance * 0.9);
    }

    const finalCameraPosition = cameraTargetPosition.clone().add(
        desiredCameraPosition.clone()
            .sub(cameraTargetPosition)
            .normalize()
            .multiplyScalar(cameraDistance)
    );

    camera.position.lerp(finalCameraPosition, 12 * delta);

    const lookAtTarget = new THREE.Vector3();
    lookAtTarget.lerpVectors(
        camera.userData.lookAt || cameraTargetPosition, 
        cameraTargetPosition, 
        12 * delta
    );
...
function updateVisualEffects(delta) {
    ...
    scene.traverse(object => {
        if (object.material && object.material.uniforms && object.material.uniforms.viewVector) {
            object.material.uniforms.viewVector.value = camera.position;
        }
    });

    if (!hookActive && (Math.abs(playerVelocity.x) > 1 || Math.abs(playerVelocity.z) > 1) && onGround) {
        if (Math.random() > 0.8) {
            createMovementTrail();
        }
    }
...
function createMovementTrail() {
    ...
    const positions = new Float32Array(3);
    positions[0] = player.position.x; positions[1] = player.position.y + 0.1; positions[2] = player.position.z;
    ...
    function animateTrail() {
        ...
        trailMaterial.opacity = 0.7 * (1 - elapsed * 2);
        trailMaterial.size = 0.3 * (1 - elapsed);
